...
const props = defineProps(['postsByUser']);
const { postsByUser } = toRefs(props);

// console.log(JSON.stringify(postsByUser))
...
<template>
    <div @mouseenter="isHover = true" @mouseleave="isHover = false"
        ...
        @click="$emit('selectedPost', postsByUser)"
        >
        ...
        <div v-if="isHover" :class="isHover ? 'bg-black bg-opacity-40' : '' "
            ...
            <div class=" flex items-center justify-around w-[50%]">
                <div class="flex items-center justify-center">
                    ...
                    <div class="pl-1">{{ postsByUser.likes.length }}</div>
                </div>
                <div class="flex items-center justify-center">
                    <Comment :size="30" fillColor="#FFFFFF" />
                    <div class="pl-1">{{ postsByUser.comments.length }}</div>
                </div>
            </div>
        </div>

        <img class="aspect-square mx-auto z-0 object-cover cursor-pointer" :src="postsByUser.file" />
    </div>
</template>
