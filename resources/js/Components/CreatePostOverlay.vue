...
const getUploadedImage = (e) => {
    ...
    fileDisplayed.value = URL.createObjectURL(e.target.files[0])
    setTimeout(() => {
        ...
        <div class="max-w-6xl h-[calc(100%-100px)] mx-auto mt-10 bg-white rounded-xl">
            ...
            <div class="w-full md:flex h-[calc(100%-55px)] rounded-xl overflow-auto">
                <div class="flex items-center bg-gray-100 w-full h-full overflow-hidden">
                    <div v-if="!fileDisplayed" class="flex flex-col items-center mx-auto">
                        <label for="file"
                        ...
                        <div v-if="!fileDisplayed && isValidFile === false"
                            class="text-red-500 text-center p-2 font-extrabold">File not accepted</div>
                    </div>

                    <img v-if="fileDisplayed && isValidFile === true" class=" max-w-[400px] p-4 mx-auto"
                        :src="fileDisplayed">
                ...
                <div id="TextAreaSection" class=" max-w-[720px] w-full relative">
                    ...
                    <div class="flex w-full max-h-[200px] bg-white border-b">
                        <textarea ref="textarea" v-model="form.text" placeholder="Write Caption..." rows="10"
                            class=" placeholder-gray-500 w-full border-0 mt-2 mb-2 z-50 focus:ring-0 text-gray-600 text-[18px]"></textarea>
                    </div>
